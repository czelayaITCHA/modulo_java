...
4. Si un ejemplo te marca error y no logras resolverlo en 10 minutos, anótalo en la sección de "Dudas para la próxima sesión" al final y continúa — no te quedes bloqueado/a.
...
Ejecuta mentalmente (sin escribir código) este escenario: tienes un arreglo `String[] tareas = new String[3];` completamente lleno, y necesitas agregar una cuarta tarea. **No puedes.** El tamaño de un arreglo es fijo desde su creación. Para resolver esto sin recrear el arreglo manualmente cada vez, Java ofrece las **colecciones** — el tema del siguiente bloque.
...
*(Respuestas al final del documento — no las mires todavía)*
